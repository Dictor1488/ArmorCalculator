...
def get_gaussian_probability(avg_pen, armor_val):
    ...
    if armor_val >= max_pen:
        return 0.0
    standard_deviation = avg_pen / 12.0
...
def call_update_gui(avg_pen, armor_val, ricochet, hit_body, hit_track, hit_gun, hit_angle_cos, alpha_dmg=(400, 400), enemy_hp=0):
    ...
    if not ricochet and hit_body:
        ...
        alpha_damage = alpha_dmg[0] if isinstance(alpha_dmg, (tuple, list)) else alpha_dmg
        kill_roll_prob = get_gaussian_probability(alpha_damage, enemy_hp)
    kill_prob = int(prob * kill_roll_prob // 100)
    gui_state.update_gui(armor_val, int(prob), ricochet, hit_body, hit_track, hit_gun, hit_angle, avg_pen, kill_prob)
...
def my_shot_result_default(cls, gunMarker, collisionsDetails, fullPiercingPower, shell, minPP, maxPP, entity):
    ...
    for cDetails in collisionsDetails:
        ...
        if cDetails.matInfo is None:
            ...
            if not isJet and cls._shouldRicochet(shell, hitAngleCos, matInfo):
                ...
                penetrationArmor = cls._computePenetrationArmor(shell, hitAngleCos, matInfo) if piercingPower > 0.0 else 0
                total_armor_val += penetrationArmor
                collectDebug(debugPiercingsList, penetrationArmor, hitAngleCos, minPiercingPower, maxPiercingPower, piercingPercent, matInfo, _SHOT_RESULT.NOT_PIERCED)
                break
            penetrationArmor = 0
            ...
            if matInfo.vehicleDamageFactor:
                ...
                if matInfo.extra and piercingPercent <= maxPP:
                    result = cls._CRIT_ONLY_SHOT_RESULT
                collectDebug(debugPiercingsList, penetrationArmor, hitAngleCos, minPiercingPower, maxPiercingPower, piercingPercent, matInfo, debugResult)
    ...
    call_update_gui(fullPiercingPower, total_armor_val, ricochet, hit_body, hit_track, hit_gun, min_hit_angle_cos, shell.armorDamage, entity.health)
    return result


def my_shot_result_modern_he(cls, gunMarker, collisionsDetails, fullPiercingPower, shell, minPP, maxPP, entity):
    total_armor_val = 0.0
    hit_body = False
    hit_track = False
    hit_gun = False
    ...
    for cDetails in collisionsDetails:
        if not isDestructible(entity, cDetails.compName):
            call_update_gui(fullPiercingPower, total_armor_val, False, hit_body, hit_track, hit_gun, min_hit_angle_cos, shell.armorDamage, entity.health)
            return result
        if not hit_track and (cDetails.compName == 0 or cDetails.compName >= 4):
            hit_track = True
        if cDetails.compName == 3:
        ...
        if matInfo is not None and (cDetails.compName, matInfo.kind) not in ignoredMaterials:
            ...
            if matInfo.vehicleDamageFactor:
                ...
                minPiercingPower = round(minPiercingPower - penetrationArmor)
                maxPiercingPower = round(maxPiercingPower - penetrationArmor)
                result = _SHOT_RESULT.GREAT_PIERCED if piercingPercent <= minPP and explosionDamageAbsorption == 0 else _SHOT_RESULT.LITTLE_PIERCED
                collectDebug(debugPiercingsList, penetrationArmor, hitAngleCos, minPiercingPower, maxPiercingPower, piercingPercent, matInfo, result)
                sendDebug(gunMarker, debugPiercingsList, minPP, maxPP, fullPiercingPower)
                call_update_gui(fullPiercingPower, total_armor_val, False, hit_body, hit_track, hit_gun, min_hit_angle_cos, shell.armorDamage, entity.health)
                return result
            ...
            if matInfo.extra and piercingPercent <= maxPP:
                collectDebug(debugPiercingsList, penetrationArmor, hitAngleCos, minPiercingPower, maxPiercingPower, piercingPercent, matInfo, cls._CRIT_ONLY_SHOT_RESULT)
                result = cls._CRIT_ONLY_SHOT_RESULT
            if matInfo.collideOnceOnly:
                ignoredMaterials.add((cDetails.compName, matInfo.kind))
...
gun_marker_ctrl._CrosshairShotResults._CrosshairShotResults__shotResultModernHE = classmethod(my_shot_result_modern_he)
